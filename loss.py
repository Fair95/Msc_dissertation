...
def weighted_dice_loss(y_true, y_pred):
    ...
    if K.int_shape(y_pred)[3] == 128:
        kernel_size = 11
    elif K.int_shape(y_pred)[3] == 256:
        kernel_size = 21
    elif K.int_shape(y_pred)[3] == 512:
        kernel_size = 21
    elif K.int_shape(y_pred)[3] == 1024:
        kernel_size = 41
    else:
...
def local_dice_coef(y_true, y_pred):
    smooth = 1.
    ...
    intersection = K.sum(y_true * K.greater(y_pred, 0.5), axis=(1,2,3))
    ...
    return K.mean( (2. * intersection + smooth) / (smooth + K.sum(K.greater(y_true,0.5), axis=(1,2,3)) + K.sum(K.greater(y_pred, 0.5), axis=(1,2,3)) )    )
